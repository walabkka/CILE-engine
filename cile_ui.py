...
INTERNAL_KEY = st.secrets.get("CILE_INTERNAL_KEY", "cile-dev-key-123")

if "CILE_API_URL" in st.secrets:
    API_URL = st.secrets["CILE_API_URL"]
else:
        ...
        if not regimen_text.strip():
            ...
            with st.spinner("Transmitting to CILE secure server..."):
                try:
                    ...
                    st.error("Server Error: Cannot connect to API. Ensure 'uvicorn api:app --reload' is running in Terminal 1.")
                except requests.exceptions.RequestException as e:
                    st.error(f"API Fault: {e}")
