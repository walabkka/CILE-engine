...
def generate_clinical_pdf(report):
    ...
    if renal:
        ...
        pdf.set_font("helvetica", size=10)
        for r in renal:
            pdf.multi_cell(w=190, h=6, text=sanitize(f"- {r['drug']}: {r['note']}"))
        pdf.ln(5)
    ...
    if interactions:
        ...
        for ddi in interactions:
            pdf.multi_cell(w=190, h=6, text=sanitize(f"[{ddi['severity']}] {ddi['drugs']}: {ddi['description']}"))
        pdf.ln(5)
    ...
    for profile in report.get("regimen_profiles", []):
        ...
        if alerts:
            ...
            pdf.set_font("helvetica", size=10)
            for alert in alerts:
                pdf.multi_cell(w=190, h=6, text=sanitize(f"  > {alert}"))
            pdf.ln(2)
