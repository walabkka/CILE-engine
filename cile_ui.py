...
def generate_clinical_pdf(report):
    ...
    pdf.cell(190, 10, "CILE Clinical Consultation Report", ln=True, align="C")
    ...
    pdf.cell(190, 10, "Patient Vitals & Demographics", ln=True)
    pdf.set_font("helvetica", size=10)
    pdf.cell(190, 6, text=sanitize(f"Age: {meta.get('patient_age')} | Gender: {meta.get('gender')}"), ln=True)
    pdf.cell(190, 6, text=sanitize(f"Weight: {meta.get('weight_kg')} kg | SrCr: {meta.get('serum_creatinine_mgdl')} mg/dL"), ln=True)
    pdf.cell(190, 6, text=sanitize(f"Estimated CrCl: {meta.get('estimated_crcl_mlmin')} mL/min"), ln=True)
    pdf.ln(5)
    ...
    if renal:
        ...
        pdf.cell(190, 10, "Renal Dose Adjustments Required", ln=True)
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("helvetica", size=10)
        for r in renal:
            pdf.multi_cell(w=190, h=6, text=sanitize(f"- {r['drug']}: {r['note']}"), align="L")
        pdf.ln(5)
    ...
    if interactions:
        pdf.set_font("helvetica", style="B", size=12)
        pdf.set_text_color(200, 0, 0)
        pdf.cell(190, 10, "Critical Drug Interactions", ln=True)
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("helvetica", size=10)
        for ddi in interactions:
            pdf.multi_cell(w=190, h=6, text=sanitize(f"[{ddi['severity']}] {ddi['drugs']}: {ddi['description']}"), align="L")
        pdf.ln(5)
        
    # Regimen Profiles
    pdf.set_font("helvetica", style="B", size=12)
    pdf.cell(190, 10, "Predictive Clinical Alerts", ln=True)
    pdf.set_font("helvetica", size=10)
    for profile in report.get("regimen_profiles", []):
        alerts = profile.get("predictive_alerts", [])
        if alerts:
            pdf.set_font("helvetica", style="B", size=10)
            pdf.cell(190, 6, text=sanitize(f"Drug: {profile.get('generic', 'Unknown').capitalize()}"), ln=True)
            pdf.set_font("helvetica", size=10)
            for alert in alerts:
                pdf.multi_cell(w=190, h=6, text=sanitize(f"  > {alert}"), align="L")
            pdf.ln(2)
