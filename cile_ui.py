...
try:
    INTERNAL_KEY = st.secrets["INTERNAL_KEY"]
except KeyError:
    st.error("System Configuration Error: Missing internal security key.")
    st.stop()
...
# --- SIDEBAR NAVIGATION ---
st.sidebar.title ("CILE v1.0.0")
page = st.sidebar.radio("Navigation", ["Clinical Analysis", "Analytics Dashboard"])

if page == "Clinical Analysis":
    st.title("Clinical Informatics Logic Engine")
    st.markdown("Enter patient age, vitals, and pharmacological regimen:")

    col_vitals, col_drugs = st.columns([1, 2])
    ...
    with col_vitals:
        ...
        weight = st.number_input("Weight (kg)", min_value=0.0, max_value=300.0, value=75.0)
        srcr = st.number_input("Serum Creatinine (mg/dL)", min_value=0.0, max_value=15.0, value=1.0)
        hba1c = st.number_input("HbA1c (%)", min_value=0.0, max_value=20.0, value=0.0)

    with col_drugs:
        # Regimen Input
        st.subheader("Regimen Input")
        ...
        for i in range(st.session_state.drug_count):
            ...
            with c2:
                ind = st.selectbox("Indication", ["", "P", "S"], key=f"ind_{i}", help="P = Primary Prevention, S = Secondary Prevention")
            if name.strip():
                regimen.append({"name": name.strip(), "indication": ind.lower() if ind else None})
        ...
        with col_add:
            if st.button("Add Drug"):
                ...
                st.rerun()
        with col_rem:
            if st.button("Remove") and st.session_state.drug_count > 1:
                st.session_state.drug_count -= 1
                st.rerun()
    ...
    # Execution
    if st.button("Initiate Clinical Analysis", type="primary", use_container_width=True):
        if not regimen:
            st.error("Enter at least one medication.")
        else:
            payload = {"patient": {"age": age, "gender": gender, "weight": weight,
                    "srcr": srcr, "hba1c": hba1c},
                "regimen": regimen
            }
            ...
            with st.spinner("Transmitting to CILE secure server..."):
                try:
                    ...
                    drug_names = [p.get("generic", "Unknown") for p in report.get("regimen_profiles", [])]
                    if len(drug_names) == 1:
                        base_name = drug_names[0].replace(' ', '_')
                        pdf_filename = f"{base_name}_Consult.pdf"
                        json_filename = f"{base_name}_fhir.json"
                    else:
                        name_parts = [n[:len(n)//2] if len(n) > 3 else n for n in drug_names]
                        base_name = "MergedReport_" + "_".join(name_parts).replace(" ", "")
                    ...
                    try:
                        ...
                        st.error("An error occurred during document compilation. Please contact CILE's builder.")
                    ...
                    st.error("An error occurred. Please contact with CILE's builder.")
