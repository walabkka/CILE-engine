...
def generate_clinical_pdf(report):
    ...
    pdf.set_x(10)
    pdf.cell(0, 6, text=sanitize(f"Age: {meta.get('patient_age')} | Gender: {meta.get('gender')}"), ln=1)
    pdf.set_x(10)
    pdf.cell(0, 6, text=sanitize(f"Weight: {meta.get('weight_kg')} kg | SrCr: {meta.get('serum_creatinine_mgdl')} mg/dL"), ln=1)
    pdf.set_x(10)
    pdf.cell(0, 6, text=sanitize(f"Estimated CrCl: {meta.get('estimated_crcl_mlmin')} mL/min"), ln=1)
    pdf.ln(5)
    ...
    if renal:
        ...
        for r in renal:
            pdf.set_x(10)
            pdf.multi_cell(w=0, h=6, text=sanitize(f"- {r['drug']}: {r['note']}"), align="L")
        pdf.ln(5)
    ...
    if interactions:
        ...
        pdf.set_text_color(200, 0, 0)
        pdf.set_x(10)
        pdf.cell(0, 10, "Critical Drug Interactions", ln=1)
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("helvetica", size=10)
        for ddi in interactions:
            pdf.set_x(10)
            pdf.multi_cell(w=0, h=6, text=sanitize(f"[{ddi['severity']}] {ddi['drugs']}: {ddi['description']}"), align="L")
        pdf.ln(5)
        
    # Regimen Profiles
    pdf.set_font("helvetica", style="B", size=12)
    pdf.set_x(10)
    pdf.cell(0, 10, "Predictive Clinical Alerts", ln=1)
    pdf.set_font("helvetica", size=10)
    for profile in report.get("regimen_profiles", []):
        ...
        if alerts:
            ...
            pdf.set_x(10)
            pdf.cell(0, 6, text=sanitize(f"Drug: {profile.get('generic', 'Unknown').capitalize()}"), ln=1)
            pdf.set_font("helvetica", size=10)
            for alert in alerts:
                pdf.set_x(10)
                pdf.multi_cell(w=0, h=6, text=sanitize(f"  > {alert}"), align="L")
            pdf.ln(2)
